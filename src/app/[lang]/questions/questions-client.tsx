...
import { filterQuestionsFromRows, parseQuestionHub, questionCategoryOptionsForHub, questionHubUiLabel } from "@/lib/knowledge-filters";
...
import { QUESTION_CATEGORY_TO_SLUG, parseQuestionCategoryParam } from "@/lib/query-param-filters";
...
const copy = {
  zh: {
    ...
    diseaseSub: "涵盖肠胃与症状护理、皮肤与被毛照护、健康与老年相关问答；严重或急性情况请尽早就医。",
  ...
  en: {
    ...
    heroHint: "Search symptoms, behavior, or health questions",
...
export default function QuestionsClient({ initialQuestions, lang }: { initialQuestions: Question[]; lang: UiLocale }) {
  const c = copy[lang];
  ...
  return (
    <div className="space-y-5 pt-10">
      ...
      <nav aria-label={c.navAria} className="flex min-h-12 flex-wrap gap-3 rounded-2xl border-0 bg-white p-4 shadow-[0_4px_20px_rgba(0,0,0,0.05)]">
        {tabs.map((t) => {
          const active = (t.key === "core" && !hub) || (t.key === "disease" && hub === "disease") || (t.key === "behavior" && hub === "behavior");
          return (
            <Link
              key={t.key}
              href={t.href}
              className={`rounded-full px-3 py-1.5 text-sm transition ${
                active ? "bg-zinc-900 text-white" : "border border-zinc-300 bg-white text-zinc-700 hover:bg-[var(--color-accent-green)]"
              }`}
            >
              {t.label}
            </Link>
      ...
      <form className="space-y-4" method="get" action={appPath(questionsPath)} acceptCharset="UTF-8">
        ...
        <SearchBar key={`${q}-${hub ?? ""}-${activeCategorySlug ?? ""}`} defaultValue={q} placeholder={c.searchPh} submitLabel={c.searchBtn} icon="🐱" size="hero" />
        ...
        <CategoryFilter
          ...
          items={categories.map((cat) => ({ value: QUESTION_CATEGORY_TO_SLUG[cat.value], label: cat.label }))}
          paramName="category"
      ...
      <aside className="app-panel rounded-2xl border border-amber-200/80 bg-amber-50/80 p-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          <Link href={withLang(lang, "/foods")} className="rounded-xl bg-white px-3 py-2 text-sm font-medium text-zinc-800">{c.canDataset}</Link>
          <Link href={withLang(lang, "/food-compare")} className="rounded-xl bg-white px-3 py-2 text-sm font-medium text-zinc-800">{c.breedWizard}</Link>
        </div>
      </aside>
