...
export default async function ZonesPage({ params }: Props) {
  ...
  const t = ui[lang];
  const zones = await getZonesWithProducts(lang);
  const productsHref = withLang(lang, "/products");
  const guideHref = withLang(lang, "/guide");

  return (
    <div className="space-y-8">
      <section className="rounded-2xl border border-zinc-200 bg-gradient-to-br from-white to-zinc-100 p-5">
        <h1 className="text-xl font-semibold tracking-tight">{t.title}</h1>
        <p className="mt-2 text-sm text-zinc-600">{t.lead}</p>
        ...
        <p className="mt-3 text-xs text-zinc-500">
          {lang === "zh" ? (
            <>
              ...
              <Link href={productsHref} className="underline">
                {t.productsLink}
              </Link>
              ；喂养与照护流程见{" "}
